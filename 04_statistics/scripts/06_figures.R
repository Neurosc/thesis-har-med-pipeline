...
# ── Figure 6: Flattening contrast (fig_glasser_flattening_contrast) ──────────
cat("\n", SEP, "\nFIGURE 6 — Flattening contrast\n", SEP, "\n", sep = "")

FLAT_CSV <- file.path(TABLES_DIR, "lmm_flattening.csv")
FLAT_DIR <- file.path(REPO_ROOT, "04_statistics", "figures")
OUT_FLAT <- file.path(FLAT_DIR, "fig_glasser_flattening_contrast.png")
dir.create(FLAT_DIR, showWarnings = FALSE, recursive = TRUE)

if (!file.exists(FLAT_CSV)) {
  cat("  [SKIP] lmm_flattening.csv not found:", FLAT_CSV, "\n")
} else {
  flat_raw <- read.csv(FLAT_CSV, stringsAsFactors = FALSE)

  # Parse "X - nonself" → "X" from the contrast column
  flat_raw$layer <- sub(" - nonself$", "", flat_raw$contrast)

  # Layer rows: those with a p.value (all 3 self-layer contrasts)
  layer_df <- flat_raw[!is.na(flat_raw$p.value),
                       c("layer", "estimate", "lower.CL", "upper.CL", "p.value")]
  names(layer_df)[c(3, 4)] <- c("ci_lo", "ci_hi")
  layer_df$is_avg <- FALSE

  # Average row: avg_flat_b / avg_flat_p from first non-NA entry; no CI available
  avg_b <- flat_raw$avg_flat_b[!is.na(flat_raw$avg_flat_b)][1]
  avg_p <- flat_raw$avg_flat_p[!is.na(flat_raw$avg_flat_p)][1]

  # Sort 3 layers ascending (most negative = strongest flattening first)
  layer_df <- layer_df[order(layer_df$estimate), ]

  if (!is.na(avg_b)) {
    avg_row  <- data.frame(layer = "Average", estimate = avg_b,
                           ci_lo = NA_real_, ci_hi = NA_real_,
                           p.value = avg_p, is_avg = TRUE,
                           stringsAsFactors = FALSE)
    plot_df <- rbind(layer_df, avg_row)
  } else {
    plot_df <- layer_df
  }

  plot_df$x_label <- factor(plot_df$layer, levels = plot_df$layer)

  SIG_COLOR <- "#E24B4A"
  NS_COLOR  <- "#999999"
  AVG_COLOR <- "#2E8B8B"

  plot_df$dot_color <- ifelse(
    plot_df$is_avg, AVG_COLOR,
    ifelse(!is.na(plot_df$p.value) & plot_df$p.value < 0.05, SIG_COLOR, NS_COLOR))
  plot_df$sig <- !plot_df$is_avg & !is.na(plot_df$p.value) & plot_df$p.value < 0.05

  fmt_p_flat <- function(p) {
    if (is.na(p)) return("")
    if (p < 0.001) return("p<0.001")
    sprintf("p=%.3f", p)
  }
  plot_df$p_label    <- sapply(plot_df$p.value, fmt_p_flat)
  plot_df$annot_lbl  <- ifelse(plot_df$sig,
                               paste0("* ", plot_df$p_label), plot_df$p_label)

  color_scale <- setNames(plot_df$dot_color, as.character(plot_df$x_label))

  fig_flat <- ggplot(plot_df, aes(x = x_label, y = estimate, color = x_label)) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray50", linewidth = 0.5) +
    geom_errorbar(data = plot_df[!is.na(plot_df$ci_lo), ],
                  aes(ymin = ci_lo, ymax = ci_hi),
                  width = 0, linewidth = 0.8) +
    geom_point(size = 4, shape = 16) +
    geom_text(aes(label = annot_lbl),
              vjust = -1.1, hjust = 0.5, size = 3.2, show.legend = FALSE) +
    scale_color_manual(values = color_scale, guide = "none") +
    scale_y_continuous(expand = expansion(mult = c(0.12, 0.22))) +
    labs(
      title    = "Self–nonself gap change under DMT (LMM flattening contrasts)",
      subtitle = "Negative = gap contracts under DMT",
      x        = NULL,
      y        = "Flattening (Δgap verum − Δgap placebo, s)"
    ) +
    theme_minimal(base_size = 11, base_family = "serif") +
    theme(
      ...
      plot.subtitle = element_text(hjust = 0.5, size = 10,
                                   family = "serif", color = "gray40"),
      axis.text.x   = element_text(size = 10),
      axis.title.y  = element_text(size = 10),
      legend.position = "none"
    )

  ggsave(OUT_FLAT, fig_flat, width = 7, height = 5, dpi = 300, bg = "white")
  cat(sprintf("Saved PNG: %s\n", OUT_FLAT))
}
