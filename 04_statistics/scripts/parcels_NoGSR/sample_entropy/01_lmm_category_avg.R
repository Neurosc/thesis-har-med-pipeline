# 01_lmm_category_avg.R — Pre vs post LMM on SampEn at the parcel level (parcels, NoGSR; self-contained).
# Mirror of intrinsic_timescale/02_lmm_category_avg.R, swapping AUC -> SampEn. Reads sampen_full_dataframe.csv (output of
# the (now-frozen) SampEn dataframe build (see README)), drops the 58 low-tSNR parcels, and fits
# the LMM at the parcel level (no averaging) with crossed random effects for subject and parcel.
# Reports the PRE->POST change (session) and Group x Session drug effect.
#
# Model (one row per subject x session x parcel):
#   sampen ~ session * group * self_layer + (1 | subject) + (1 | parcel)
#   subject = random intercept; parcel = random intercept (crossed)
...
#
# Run from repo root:  Rscript 04_statistics/scripts/parcels_NoGSR/sample_entropy/01_lmm_category_avg.R
...
LAYER_LEVELS  <- c("somatomotor", "Interoception", "Exteroception", "Cognition")
...
cat("\n", SEP, "\nPART 2 — Combined model: sampen ~ session * group * self_layer + (1 | subject) + (1 | parcel)\n", SEP, "\n", sep = "")
contrasts(df$self_layer) <- contr.sum(nlevels(df$self_layer))
m <- try_fit(sampen ~ session * group * self_layer + (1 | subject) + (1 | parcel), "combined", df)
...
# Multiple-comparison correction across the 4 per-layer drug-effect contrasts (one family).
# p_holm = Holm-Bonferroni (FWER control); p_fdr = Benjamini-Hochberg (FDR). The omnibus
# session:group F-test (catavg_3way_interaction_Ftest.csv) is a single test, NOT in this family.
...
cat("  p = raw | p_holm = Holm-Bonferroni (FWER) | p_fdr = Benjamini-Hochberg (FDR); family = 4 layers\n")
...
# ── Extended diagnostics: colored by self_layer and subject ───────────────────
...
CAT_COLORS <- c(somatomotor   = "#2E8B8B",
...
p_qq_cat <- ggplot(diag_df, aes(x = qq_theo, y = resid, color = self_layer)) +
  geom_point(size = 0.7, alpha = 0.35) +
  geom_abline(color = "black", linetype = "dashed", linewidth = 0.5) +
  scale_color_manual(values = CAT_COLORS) +
  facet_wrap(~ self_layer, nrow = 1, scales = "free") +
  labs(title = "QQ — by Category", x = "Theoretical quantile", y = "Residual") +
  facet_theme

p_rvf_cat <- ggplot(diag_df, aes(x = fitted_val, y = resid, color = self_layer)) +
  geom_point(size = 0.7, alpha = 0.35) +
...
ggsave(file.path(FIGS_DIR, "SampEn_QC_residuals_by_category.png"),
       p_qq_cat / p_rvf_cat, width = 14, height = 8, dpi = 300, bg = "white")
cat(sprintf("Saved: %s\n", file.path(FIGS_DIR, "SampEn_QC_residuals_by_category.png")))
...
ggsave(file.path(FIGS_DIR, "SampEn_QC_residuals_by_subject.png"),
       p_subj, width = 14, height = 5, dpi = 300, bg = "white")
cat(sprintf("Saved: %s\n", file.path(FIGS_DIR, "SampEn_QC_residuals_by_subject.png")))

extreme <- diag_df[order(-diag_df$abs_resid),
                   c("subject", "group", "session", "self_layer", "parcel",
                     "sampen", "fitted_val", "resid", "abs_resid")]
extreme <- head(extreme, 30); rownames(extreme) <- NULL
write.csv(extreme, file.path(TABLES_DIR, "SampEn_QC_extreme_residuals.csv"), row.names = FALSE)
cat(sprintf("Saved: %s\n", file.path(TABLES_DIR, "SampEn_QC_extreme_residuals.csv")))
cat("\nTop-30 extreme residuals (|resid| descending):\n")
print(extreme[, c("subject", "self_layer", "parcel", "resid")], row.names = FALSE, digits = 3)
cat("\nExtreme residuals by Category:\n"); print(table(extreme$self_layer))
cat("Extreme residuals by Subject:\n");   print(sort(table(extreme$subject), decreasing = TRUE))

cat("\n", SEP, "\nSUMMARY\n", SEP, "\n", sep = "")
cat("Drug effect per layer (verum vs placebo, in pre->post)  -> catavg_drug_effect.csv\n")
cat("Pre-post change per arm                                 -> catavg_prepost_by_arm.csv\n")
cat("Drug effect differs across layers? (3-way F-test)       -> catavg_3way_interaction_Ftest.csv\n")
cat("\n", SEP, "\nDONE\n", SEP, "\n", sep = "")
