# 05_qc_baseline_balance.R — Baseline balance check (placebo vs verum at ses-01)
#
...
run_tests <- function(plac, verm) {
  ...
  list(t = unname(t_r$statistic), df = unname(t_r$parameter), t_p = t_r$p.value,
       U = unname(mw_r$statistic), U_p = mw_r$p.value,
       d = dv, d_lab = d_label(dv))
}
...
cat_tbl <- do.call(rbind, cat_rows); rownames(cat_tbl) <- NULL
