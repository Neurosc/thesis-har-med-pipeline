# 02_lmm.R — Self vs nonself LMM (Glasser parcels, no G1 covariate)
#
# Combines Keskin et al. self parcels (Glasser-based, per-layer) with the
# nonself Glasser parcels. Formula: auc ~ group * session * self_layer + RE
#
# Layer variable (self_layer):
#   "Interoception"  — per atlas
#   "Exteroception"  — per atlas
#   "Cognition"      — per atlas
#   "nonself"        — all remaining Glasser parcels (tSNR-filtered)
#
# Reference level: "nonself"
#
# Run from repo root:
...
OUT_DATA  <- file.path(TABLES_DIR, "glasser_self_nonself_model_ready.csv")
OUT_FE    <- file.path(TABLES_DIR, "glasser_self_lmm_fixed_effects.csv")
OUT_EMM   <- file.path(TABLES_DIR, "glasser_self_lmm_emm_contrasts.csv")
OUT_FLAT  <- file.path(TABLES_DIR, "glasser_self_lmm_flattening.csv")
OUT_FIG24 <- file.path(FIGS_DIR,   "fig24_glasser_self_lmm_diagnostics.png")
...
try_fit <- function(formula, label, data = df) {
  tryCatch(
    withCallingHandlers(
...
ggsave(OUT_FIG24, p_qq | p_rvf | p_hist, width = 12, height = 4,
       dpi = 300, bg = "white")
cat(sprintf("Saved: %s\n", OUT_FIG24))
...
OUT_FIG_TRIM <- file.path(FIGS_DIR, "fig_glasser_lmm_diagnostics_trimmed.png")
...
m_trim <- try_fit(FORMULA_FULL, "trim/full-RE", data = df_trim)
if (!is.null(m_trim) && isSingular(m_trim)) {
  cat("  Singular — retrying with (1|subject)+(1|roi_uid)...\n")
  m_trim <- try_fit(FORMULA_SIMP, "trim/simple-RE", data = df_trim)
}
...
resids_trim <- resid(m_trim); fitted_trim <- fitted(m_trim)
qq_smp_t <- sort(sample(resids_trim, min(5000L, length(resids_trim))))
qq_teo_t <- qnorm(ppoints(length(qq_smp_t)))
rvf_i    <- sample(length(resids_trim), min(5000L, length(resids_trim)))

p_qq_t <- ggplot(data.frame(theoretical = qq_teo_t, sample = qq_smp_t),
                 aes(x = theoretical, y = sample)) +
  geom_point(size = 0.8, alpha = 0.35, color = "steelblue") +
  geom_abline(color = "red", linetype = "dashed") +
  labs(title = "QQ Plot (trimmed)", x = "Theoretical", y = "Sample") + diag_theme
p_rvf_t <- ggplot(data.frame(fitted = fitted_trim[rvf_i], resid = resids_trim[rvf_i]),
                  aes(x = fitted, y = resid)) +
  geom_point(size = 0.8, alpha = 0.25, color = "steelblue") +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Resid vs Fitted (trimmed)", x = "Fitted", y = "Residuals") + diag_theme
p_hist_t <- ggplot(data.frame(resid = resids_trim), aes(x = resid)) +
  geom_histogram(bins = 80, fill = "steelblue", color = "white", linewidth = 0.2) +
  labs(title = "Histogram (trimmed)", x = "Residual", y = "Count") + diag_theme

ggsave(OUT_FIG_TRIM, p_qq_t | p_rvf_t | p_hist_t, width = 12, height = 4,
       dpi = 300, bg = "white")
cat(sprintf("\nSaved: %s\n", OUT_FIG_TRIM))
