#!/usr/bin/env python3
"""
DSE/DVARS QC panels — faithful replication of Mu Reinwald / Afyouni fMRIDiag_plot.m.

Generates a four-panel diagnostic figure per run:
  Panel 1: Motion (FD, |Rotation|, |Translation|) + √D-var right axis
  Panel 2: DSE variance components (√A-var, √D-var, √S-var, E-var) + % of A-var
  Panel 3: Δ%D-var with practically significant bands
  Panel 4: Voxel carpet plot (grayscale, per-voxel z-scored)

Subject schedule
----------------
  sub-01  ses-01, ses-02 — raw + denoised (+GSR, -GSR) for sub-01 ses-01 only
  sub-12  ses-01, ses-02 — raw only
  sub-21  ses-01, ses-02 — raw only
  sub-39  ses-01, ses-02 — raw only

Output files (01_denoising/figures/dse_panels/)
-----------------------------------------------
  {sub}_{ses}_dse_raw.png / .pdf       — all 8 runs
  sub-01_ses-01_dse_GSR.png / .pdf     — sub-01 ses-01 +GSR denoised
  sub-01_ses-01_dse_NoGSR.png / .pdf   — sub-01 ses-01 -GSR denoised
...
TR        = 1.8    # seconds
FD_THRESH = 0.3    # mm (censoring threshold; Panel 1 reference line is 0.5 mm)
...
def _load_masked_bold(bold_path, mask_path):
    """Load NIfTI BOLD and flatten with brain mask. Returns (n_vox, T) float32."""
    bold_img  = nib.load(str(bold_path))
    bold_data = bold_img.get_fdata(dtype=np.float32)
    mask_flat = nib.load(str(mask_path)).get_fdata().astype(bool).ravel()
    T = bold_data.shape[3]
    Y = bold_data.reshape(-1, T)
    return Y[mask_flat].astype(np.float32)   # (n_vox, T)


def _compute_abs_mov(conf_df):
    """
    Absolute motion components for Panel 1, both in mm.
    ...
    abs_rot   = np.abs(d_rot).sum(axis=1) * 50.0     # rad × 50 mm sphere → mm
...
def _make_and_save(V_dse, dvars_stat, fd, abs_mov, Y_carpet,
                   title, stem):
    """Build the 4-panel figure and save as .png and .pdf."""
    fig = fmri_diag_plot(
        V_dse          = V_dse,
        ...
        title          = title,
        figsize        = (10, 12),
        fd_thresh      = 0.5,     # visual reference line (spec says 0.5 mm)
        prac_sig_thresh= 5.0,
    )
    for ext in ('png', 'pdf'):
...
def main():
    ...
    for sub, ses in RUNS:
        ...
        tag = f"{sub}_{ses}"
        _make_and_save(V_raw, dstat_raw, fd_ts, abs_mov, Y_raw,
                       title=f"{sub} {ses} — raw",
                       stem=f"{tag}_dse_raw")
        ...
        if sub == 'sub-01' and ses == 'ses-01':
            ...
            if den['gsr'].exists():
                ...
                _make_and_save(V_gsr, dstat_gsr, fd_ts, abs_mov, Y_gsr,
                               title=f"{sub} {ses} — +GSR denoised",
                               stem=f"{tag}_dse_GSR")
            else:
                print(f"  +GSR denoised file not found, skipping: {den['gsr']}")

            if den['nogsr'].exists():
                print(f"  Loading -GSR denoised BOLD ...")
                Y_nogsr = _load_masked_bold(den['nogsr'], mask_path)
                V_nogsr, dstat_nogsr = _run_dse_dvars(Y_nogsr, '-GSR')
                _make_and_save(V_nogsr, dstat_nogsr, fd_ts, abs_mov, Y_nogsr,
                               title=f"{sub} {ses} — -GSR denoised",
                               stem=f"{tag}_dse_NoGSR")
            else:
                print(f"  -GSR denoised file not found, skipping: {den['nogsr']}")
