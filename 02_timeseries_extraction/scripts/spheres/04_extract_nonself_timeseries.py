#!/usr/bin/env python3
"""
Nonself ROI timeseries extraction — DMT-MED dataset.

Extracts mean BOLD timeseries for each Glasser nonself ROI across all
35 included subjects × 2 sessions × 3 BOLD versions = 210 runs.
...
Atlas: nonself_atlas_native.nii.gz (Glasser-derived, overlap-cleaned,
       resampled to native BOLD grid 1.72x1.72x2.00mm)

Outputs
-------
  02_timeseries_extraction/results/timeseries_nonself/{version}/
      sub-XX_ses-YY_nonself_timeseries.csv  (rows=timepoints, cols=ROI numbers)
  02_timeseries_extraction/results/timeseries_nonself/_extraction_log.tsv

Run on server:
  conda activate fmri
  python 02_timeseries_extraction/scripts/04_extract_nonself_timeseries.py
"""
...
OUT_ROOT      = REPO_ROOT / "02_timeseries_extraction" / "results" / "timeseries_nonself"
...
def out_csv_path(sub, ses, version):
    return OUT_ROOT / version / f"{sub}_{ses}_nonself_timeseries.csv"
