...
print(
    "Computing custom FD with respiratory filter [0.2-0.5 Hz, clipped to Nyquist] "
...
# Step 4: Master summary table — all columns explicitly suffixed, no ambiguity
TSV_COLUMNS = [
...
# Step 5: Group-level figure (uses custom FD throughout)
SESSION_COLORS = {"ses-01": "#4878CF", "ses-02": "#D65F5F"}

fig, axes = plt.subplots(2, 2, figsize=(12, 9))
fig.suptitle("Group Motion QC Summary (custom FD)", fontsize=14)

# A: histogram of mean custom FD
axes[0, 0].hist(df["mean_fd_custom"], bins=20, color="steelblue", edgecolor="white")
for thresh, ls in [(0.2, "--"), (0.3, "-.")]:
    axes[0, 0].axvline(thresh, color="black", linestyle=ls, linewidth=1,
                       label=f"{thresh} mm")
axes[0, 0].set_xlabel("Mean FD — custom (mm)")
axes[0, 0].set_ylabel("Count")
axes[0, 0].set_title("Distribution of mean FD (custom)")
axes[0, 0].legend(fontsize=8)

# B: histogram of % censored (custom FD)
axes[0, 1].hist(df["pct_censored_custom"], bins=20, color="darkorange", edgecolor="white")
for thresh, ls in [(10, "--"), (25, "-."), (50, ":")]:
    axes[0, 1].axvline(thresh, color="black", linestyle=ls, linewidth=1,
                       label=f"{thresh}%")
axes[0, 1].set_xlabel("% censored frames (custom FD)")
axes[0, 1].set_ylabel("Count")
axes[0, 1].set_title("Distribution of % censored frames (custom FD)")
axes[0, 1].legend(fontsize=8)

# C: scatter mean custom FD vs % censored, coloured by session
for ses, grp in df.groupby("session"):
    axes[1, 0].scatter(grp["mean_fd_custom"], grp["pct_censored_custom"],
                       label=ses, color=SESSION_COLORS.get(ses, "gray"),
                       alpha=0.8, edgecolors="white", linewidths=0.5)
axes[1, 0].set_xlabel("Mean FD — custom (mm)")
axes[1, 0].set_ylabel("% censored frames (custom FD)")
axes[1, 0].set_title("Mean FD vs % censored (custom FD)")
axes[1, 0].legend(fontsize=8)

# D: boxplot mean custom FD by session
sessions_ordered = sorted(df["session"].unique())
data_by_session = [df.loc[df["session"] == s, "mean_fd_custom"].values
...
for patch, ses in zip(bp["boxes"], sessions_ordered):
    ...
    patch.set_alpha(0.7)
axes[1, 1].set_ylabel("Mean FD — custom (mm)")
axes[1, 1].set_title("Mean FD (custom) by session")
...
# FD comparison figure: fMRIPrep vs custom (reads directly from TSV columns)
...
# Step 6: Final console summary — explicitly uses custom FD
total = len(summaries)
n_failed = len(failed)
print(f"\n── Final summary (FD source: custom Goldberg/Lynch) ──")
print(f"  Runs processed successfully : {total}")
print(f"  Runs failed                 : {n_failed}")
if failed:
    print(f"  Failed runs: {', '.join(failed)}")

pct = df["pct_censored_custom"]
print(f"\n  Censored frame distribution (custom FD, threshold = {FD_THRESHOLD} mm):")
...
mfd = df["mean_fd_custom"]
print(f"\n  Mean FD distribution (custom):")
...
print(f"\n  Exclusion suggestion (by % censored, custom FD):")
